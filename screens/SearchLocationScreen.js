...
export default function SearchLocationScreen({ route, navigation }) {
  ...
  const handleSelectLocation = async (location) => {
    try {
      ...
      // Check if the new location is already saved (liked)
      const savedData = await AsyncStorage.getItem('savedLocations');
      const savedLocations = savedData ? JSON.parse(savedData) : [];
      const isSaved = savedLocations.some(
        (loc) =>
          loc.name === data.location.name &&
          loc.latitude === data.location.lat &&
          loc.longitude === data.location.lon
      );
      setIsLiked(isSaved);
    } catch (error) {
      console.error('Failed to fetch weather data:', error);
  ...
  // Toggle between the liked state to determine whether the location has been saved
  ...
  return (
    ...
    {/* CHoose the background based on the weather condition */}
      <Video
        source={videoSource}
        rate={1.0}
        volume={1.0}
      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
        ...
        {/* Display the icon provided by the API */}
        <Image
          style={styles.weatherIcon}
          source={{ uri: `https:${weatherData.current.condition.icon}` }}
        />
        {/* Display the current condition and temperature */}
        ...
        {/* Display the sunrise, sunset, moon phase */}
